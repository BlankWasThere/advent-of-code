...
pub fn solve(input: &str) -> anyhow::Result<()> {
    ...
    for edge in edges.into_iter() {
        ...
        match (circuit_a, circuit_b) {
            (None, None) => {
                prev_a = Some(Rc::clone(&a));
                prev_b = Some(Rc::clone(&b));
            ...
            (Some(circuit_a), None) => {
                ...
                circuit_a.borrow_mut().connections.push(b)
            }
            (None, Some(circuit_b)) => {
                prev_a = Some(Rc::clone(&a));
                prev_b = Some(Rc::clone(&b));

                circuit_b.borrow_mut().connections.push(a)
            }
    ...
    // Calculate product
    let (x1, x2) = if let Some(a) = prev_a
        && let Some(b) = prev_b
    {
        (a.x, b.x)
    } else {
        return Err(anyhow::anyhow!("Insufficient junction boxes."));
    };

    println!("Answer: {}", x1 * x2);
